...
def simulate_drafts(config: Config, num_runs: int):
    ...
    for run_idx in range(num_runs):
        ...
        env.draft_order = env._generate_snake_draft_order(env.config.draft.NUM_TEAMS, env.total_roster_size_per_team)
        ...
        while not done:
            ...
            if current_team_id == env.agent_team_id:
                ...
                with torch.no_grad():
                    if config.training.ENABLE_ACTION_MASKING:
            ...
            for team_id_check in range(1, config.draft.NUM_TEAMS + 1):
        ...
        run_results = {
            'draft_log': draft_log,
            'final_team_rosters': {team_id: env.teams_rosters[team_id]['PLAYERS'] for team_id in range(1, config.draft.NUM_TEAMS + 1)}
        }
    ...
    for run_idx, result in enumerate(all_simulation_results):
        ...
        for team_id, players in result['final_team_rosters'].items():
            scores = calculate_roster_scores(players, config.draft.ROSTER_STRUCTURE, config.draft.BENCH_MAXES)
    ...
    for team_id in sorted(all_runs_team_scores.keys()):
        ...
        if team_id == config.draft.AGENT_START_POSITION:
    ...
    if config.draft.NUM_TEAMS > 1: # Only calculate average opponent if there are opponents
        # Calculate the average opponent score based on all opponent teams, excluding the agent's team
        total_opponent_teams = config.draft.NUM_TEAMS - 1
...
if __name__ == "__main__":
    config = Config()
    simulate_drafts(config, config.training.NUM_SIMULATION_RUNS)