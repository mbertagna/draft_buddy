...
class CheckpointManager:
    ...
    def _validate_config(self, loaded_config: dict, current_config: "Config", is_training: bool):
        """Validate loaded config values against runtime config."""
        ...
        if set(saved_features) != set(current_features):
            raise ValueError(
                "Configuration mismatch: ENABLED_STATE_FEATURES are different. "
        ...
        if saved_teams != current_teams:
            if is_training:
                raise ValueError(
                    "Configuration mismatch: NUM_TEAMS is different. "
                    f"Checkpoint trained on {saved_teams}, current config has {current_teams}."
                )
            print(
                f"Warning: NUM_TEAMS mismatch. Model trained for {saved_teams}, but draft is for "
                f"{current_teams}. AI results may be suboptimal."
            )
        if is_training:
            mismatched_params = []
            for parameter in ["LEARNING_RATE", "DISCOUNT_FACTOR", "HIDDEN_DIM"]:
                saved_value = loaded_config.get("training", {}).get(parameter)
                current_value = getattr(current_config.training, parameter)
                if saved_value is not None and saved_value != current_value:
                    mismatched_params.append(
                        f"{parameter}: Checkpoint={saved_value}, Current={current_value}"
                    )
            if mismatched_params:
                details = ", ".join(mismatched_params)
                raise ValueError(
                    f"Configuration mismatch: Key training hyperparameters differ. Details: {details}"
                )

    def save_checkpoint(self, run_version_dir: str, episode: int, config: "Config") -> str:
        """Save a consolidated checkpoint file."""
        ...
        checkpoint_data = {
            ...
            "value_state_dict": (
                self._value_network.state_dict() if self._value_network is not None else None
            ),
    ...
    def load_checkpoint(self, filepath: str, config: "Config", is_training: bool = False) -> int:
        """Load, validate, and apply checkpoint state.
