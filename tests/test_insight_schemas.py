...
def _sample_bullet(url: str = "https://espn.com/article") -> EvidenceBullet:
...
def test_outlook_phrase_rejects_more_than_eight_words() -> None:
    """Verify outlook_phrase enforces the eight-word limit."""
    with pytest.raises(ValueError, match="8 words"):
        PlayerInsight(
            outlook_phrase="one two three four five six seven eight nine",
...
def test_sanitize_synthesis_payload_truncates_long_bullets() -> None:
    ...
    payload = sanitize_synthesis_payload(
        {
            "outlook_phrase": "Elite upside with workload concerns",
    ...
    assert len(insight.bullets[0].text) == 120
    assert insight.summary == "First sentence. Second sentence."
    assert insight.outlook_phrase == "Elite upside with workload concerns"
